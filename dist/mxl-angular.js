/**
 * @license mxl-angular v0.9.0
 * (c) 2016 Sebis
 * License: Sebis Proprietary
 * https://bitbucket.org/sebischair/mxl-angular
 */
...
(function (mod) {
    ...
    function getText(completion) {
        return "" + (completion.addApostrophes ? "'" : "") + completion.name + (completion.addApostrophes ? "'" : "") + (completion.addOpeningBracket ? "(" : "");
    }
    ...
    function Widget(completion, data) {
        ...
        var hintsDialog = this.hintsDialog = document.createElement("div");
        hintsDialog.className = "CodeMirror-hintsDialog"


        var hints = this.hints = document.createElement("ul");
        hints.className = "CodeMirror-hints";

        var hintHelp = this.hintHelp = document.createElement("div");
        hintHelp.className = "CodeMirror-hintHelp"

        hintsDialog.appendChild(hintHelp);
        hintsDialog.appendChild(hints);
        ...
        for (var i = 0; i < completions.length; ++i) {
            ...
            if (cur.type != null) className = "mxl-" + cur.type + " " + className;
            elt.className = className;
            elt.appendChild(document.createTextNode(cur.name));
            elt.hintId = i;
        }

        var pos = cm.cursorCoords(completion.options.alignWithWord ? data.from : null);
        var left = pos.left, top = pos.bottom, below = true;
        hintsDialog.style.left = left + "px";
        hintsDialog.style.top = top + "px";
        // If we're at the edge of the screen, then we want the menu to appear on the left of the cursor.
        var winW = window.innerWidth || Math.max(document.body.offsetWidth, document.documentElement.offsetWidth);
        var winH = window.innerHeight || Math.max(document.body.offsetHeight, document.documentElement.offsetHeight);
        (completion.options.container || document.body).appendChild(hintsDialog);
        var box = hintsDialog.getBoundingClientRect(), overlapY = box.bottom - winH;
        if (overlapY > 0) {
            var height = box.bottom - box.top, curTop = pos.top - (pos.bottom - box.top);
            if (curTop - height > 0) { // Fits above cursor
                hintsDialog.style.top = (top = pos.top - height) + "px";
                below = false;
            } else if (height > winH) {
                hintsDialog.style.height = (winH - 5) + "px";
                hintsDialog.style.top = (top = pos.bottom - box.top) + "px";
                var cursor = cm.getCursor();
                if (data.from.ch != cursor.ch) {
                    pos = cm.cursorCoords(cursor);
                    hintsDialog.style.left = (left = pos.left) + "px";
                    box = hintsDialog.getBoundingClientRect();
                }
            }
        }
        ...
        if (overlapX > 0) {
            ...
            hintsDialog.style.left = (left = pos.left - overlapX) + "px";
        }
        ...
        CodeMirror.on(data, "select", function (completion, element) {
            hintHelp.innerHTML = "<h4>" + (completion.helpHeader || completion.name) + "</h4>" + ((completion.description && completion.description != "") ? completion.description : "<i>No documentation</i>");
        });
    ...
    Widget.prototype = {
        close: function () {
            ...
            this.hintsDialog.parentNode.removeChild(this.hintsDialog);
...
(function(mod) {
  ...
  function startLinting(cm, currentError) {
    var state = cm.state.lint, options = state.options;
    var passOptions = options.options || options; // Support deprecated passing of `options` property in options
    if (options.async || options.getAnnotations.async)
        options.getAnnotations(cm.getValue(), updateLinting, passOptions, cm, currentError);
    else
        updateLinting(cm, options.getAnnotations(cm.getValue(), passOptions, cm, currentError));
  }
  ...
  function onChange(cm, currentError) {
    var state = cm.state.lint;
    clearTimeout(state.timeout);
    state.timeout = setTimeout(function(){startLinting(cm, currentError);}, state.options.delay || 500);
  }
